...
//uniform vec4 textureRatios_0_to_3;			
		...
		// Sample the textures
		vec4 texColor00 = texture(textures00, vertexUV.st); // Brick wall texture
		vec4 texColor01 = texture(textures01, vertexUV.st); // Bullet hole texture
		vec4 texColor02 = texture(textures02, vertexUV.st); // Additional textures as needed
		vec4 texColor03 = texture(textures03, vertexUV.st); // Additional textures as needed

		// Define a threshold to determine what constitutes white in texColor01
		float whiteThreshold = 0.9; // Adjust as necessary
		float whiteDiff = max(max(texColor01.r, texColor01.g), texColor01.b);

		// Blend textures based on ratios
		vec3 blendedColor = texColor00.rgb * textureRatios_0_to_3.x +
							texColor01.rgb * textureRatios_0_to_3.y +
							texColor02.rgb * textureRatios_0_to_3.z +
							texColor03.rgb * textureRatios_0_to_3.w;

		// Check if the bullet hole texture is white
		if (whiteDiff > whiteThreshold) {
			// Use texColor00 (brick wall) if texColor01 (bullet hole) is white
			pixelColour.rgb = texColor00.rgb;
		}	
		else {
			// Mix texColor00 and blendedColor based on texColor01 alpha
			vec3 finalColor = mix(texColor00.rgb, blendedColor, texColor01.a);
			
			pixelColour.rgb = finalColor;
		}
